...
In this post, we briefly introduce the physical theory of phase transitions.

## Ising Model

Consider the Ising model in \( d \) dimensions on a hypercubic lattice of side length \( L \), with  
\[
N = L^d
\]
lattice sites. At each site \( i \) there is a spin variable
\[
s_i \in \{+1, -1\}.
\]
...
where:
\begin{itemize}
  \item \( \langle i j \rangle \) denotes nearest-neighbor pairs,
  \item \( B \) is the external magnetic field.
\end{itemize}
...
We model the system with a coarse-grained field \( m(x) \) that encodes information about the spins
\( s_i \) in a neighborhood of position \( x \).

The local free-energy functional consistent with \( \mathbb{Z}_2 \) symmetry is
\[
F[m(\mathbf{x})]
=
\int d^d x
...
Assume that fluctuations are small, so that
\[
m(x) = m
\]
...
\begin{itemize}
\item If \( a(T) > 0 \), the only solution is
\[
m = 0 ,
\]
corresponding to the symmetric phase.

\item If \( a(T) < 0 \), there are two symmetry-related solutions
\[
m = \pm m_0,
\qquad
...
Define the critical temperature \( T_c \) by
\[
a(T_c) = 0 .
\]
This defines a *second-order phase transition*.
...
the heat capacity has a finite discontinuity at \( T = T_c \),
which is the hallmark of a second-order phase transition.
...
Then for \( T < T_c \),
\[
m_0 = \sqrt{\frac{a_0}{2b_0}} (T_c - T)^{1/2}.
\]
...
Mean-field theory does not capture spatial fluctuations.
These fluctuations are described by correlation functions
\[
\langle \phi(x_1)\phi(x_2)\cdots\phi(x_n)\rangle ,
\]
where we switch notation from the magnetization \( m \) to a scalar field
\( \phi \) to emphasize the connection with quantum field theory.
...
By Fourier transforming and completing the square in the Gaussian functional
integral, the generating functional is
\[
Z_0[J]
=
...
Arbitrary correlation functions are obtained by functional differentiation:
\[
...
short-range order.  
At distances \( r \ll \xi \), correlations decay algebraically, reflecting
